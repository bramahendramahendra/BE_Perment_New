...
// GenerateIDPengajuan membuat ID unik untuk pengajuan berdasarkan kostl, tahun, triwulan, dan timestamp saat ini.
...
// GenerateIDDetail membuat ID untuk baris detail (format: <idPengajuan>P<index>).
func GenerateIDDetail(idPengajuan string, index int) string {
	return fmt.Sprintf("%sP%03d", idPengajuan, index+1)
}

// GenerateIDSubDetail membuat ID untuk baris sub-detail (format: <idPengajuan>C<index>).
